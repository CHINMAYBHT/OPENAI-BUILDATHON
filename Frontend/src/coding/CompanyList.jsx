...
function CompanyList() {
  ...
  // Filter companies based on search term and category using service
  const filteredCompanies = useMemo(() => {
    const filtered = companyService.filterCompanies(searchTerm, selectedCategory);
      ...
      <main className="pt-24 pb-16 px-6 sm:px-8 lg:px-12">
        <div className="w-full">
                      ...
                      <div className="px-4 py-2">
                        ...
                        {categories.map(category => (
                          <button
                            ...
                            className={`block w-full text-left px-2 py-1 text-sm rounded hover:bg-gray-100 ${
                              selectedCategory === category.id ? 'bg-blue-100 text-blue-700' : 'text-gray-700'
                            }`}
                          >
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div
              key={company.id}
              className="group bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-100 hover:-translate-y-1"
            >
              <div className="p-6">
                {/* Company Header */}
                <div className="flex items-start justify-between mb-4">
                  <div>
                    ...
                    <span className={`inline-block text-xs font-medium ${getCategoryColor(company.category)}`}>
                      {categories.find(cat => cat.id === company.category)?.label}
                    </span>
                  </div>
                ...
                <div className="mb-4">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <div className={`px-2 py-1 rounded text-center ${getDifficultyColor('easy')}`}>
                      <div className="text-base font-bold">{company.difficulty.easy}</div>
                      <div className="text-sm">Easy</div>
                    </div>
                    <div className={`px-2 py-1 rounded text-center ${getDifficultyColor('medium')}`}>
                      <div className="text-base font-bold">{company.difficulty.medium}</div>
                      <div className="text-sm">Medium</div>
                    </div>
                    <div className={`px-2 py-1 rounded text-center ${getDifficultyColor('hard')}`}>
                      <div className="text-base font-bold">{company.difficulty.hard}</div>
                ...
                <div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {company.popularTags.slice(0, 4).map((tag, idx) => (
                      <span
                        key={idx}
                ...
                <Link
                  ...
                  className="w-full bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white py-3 rounded-lg font-semibold text-sm transition-all duration-300 flex items-center justify-center shadow-md hover:shadow-lg group-hover:scale-105"
                >
