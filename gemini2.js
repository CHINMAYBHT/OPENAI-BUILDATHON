...
// Skill Gap Analysis with Gemini AI
router.post("/analyze-skill-gap", async (req, res) => {
  try {
    ...
    const systemInstruction = `You are an expert Career Coach and Technical Recruiter.
Analyze the provided Job Description and Resume to perform a detailed Skill Gap Analysis.

Hash out the differences between the required skills and the candidate's current skills.

Return a STRICT JSON object with the following structure:
{
  "matchPercentage": <number 0-100>,
  "summary": "<concise summary of fit 2-3 sentences>",
  "statistics": {
  ...
  "recommendedRoles": [
    {
      "role": "<Role Title>",
      "match": "<High/Medium>",
      "reason": "<Short reason based on resume skills>",
      "linkedinUrl": "https://www.linkedin.com/jobs/search/?keywords=<Role Title encoded>",
      "indeedUrl": "https://www.indeed.com/jobs?q=<Role Title encoded>"
    }
  ],
  "skillBreakdown": {
    "matched": [
      { "skill": "<name>", "category": "<category>", "proficiency": "<Beginner/Intermediate/Advanced>" }
    ],
    "gaps": [
      {
        "skill": "<name>",
        ...
        "category": "<category>",
        "estimatedHours": <number of learning hours needed>,
        "courses": [
           {
             "name": "<Course Name>",
  ...
  "recommendedCourses": [
    {
      "skill": "<related gap skill>",
      "name": "<Specific Course Name>",
      "platform": "<Coursera/Udemy/Pluralsight/edX/Other>",
      "hours": "<estimated hours>",
      "type": "<Course/Tutorial/Documentation>",
      "url": "<SEARCH URL for the platform (e.g. search?q=name)>"
    }
  ],
  "recommendedProjects": [
    {
  ...
  "estimatedTimeline": "<Overall estimated time>"
}

IMPORTANT RULES:
1. Be strict but fair with the match percentage.
2. **RECOMMENDED ROLES**: Based on the RESUME (not the job desc), suggest 2-3 roles the candidate is CURRENTLY well-suited for.
3. **COURSES**: To ensure 100% link reliability, ALWAYS provide a **SEARCH URL** for the course name on the target platform. Do NOT try to guess specific course page URLs.
   - Coursera: https://www.coursera.org/search?query=<Course Name>
   - Udemy: https://www.udemy.com/courses/search/?q=<Course Name>
   - edX: https://www.edx.org/search?q=<Course Name>
   - Pluralsight: https://www.pluralsight.com/search?q=<Course Name>
4. **LEARNING PATH**: Make it detailed and actionable.
...
${resume}
    ...
    // --- RAG IMPLEMENTATION: Retrieve Internal Problems ---
    try {
      ...
      console.error("RAG Enrichment Error:", ragError);
      // Don't fail the whole request if RAG fails, just proceed without recommendations
    }
    // -----------------------------------------------------
